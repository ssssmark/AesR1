...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        #max_scores=[x["max_score"] for x in inputs]
    ...
    def _get_per_token_entropy(
        ...
        **custom_multimodal_inputs,
        ...
        next_token_ids = input_ids[:, 1:]                # (B, L-1)
        B, T, V = logits.size()
        per_token_entropy = torch.empty(B, T, device=logits.device)

        for b in range(B):
            # (T, V)
            log_probs = logits[b].log_softmax(dim=-1)
            probs     = log_probs.exp()
            entropy_row = -(probs * log_probs).sum(dim=-1)     # (T,)
            per_token_entropy[b] = entropy_row
        return per_token_entropy , next_token_ids       # (B, L-1)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        per_token_entropy,ids = self._get_per_token_entropy(model, input_ids, attention_mask, **multimodal_inputs)
        per_token_entropy = per_token_entropy[:, prompt_ids.size(1) - 1:]
        ids=ids[:, prompt_ids.size(1) - 1:]
        step_entropy = (per_token_entropy * completion_mask).sum() / completion_mask.sum()
        self._metrics["avg_entropy"].append(
            self.accelerator.gather_for_metrics(step_entropy).mean().item()
        )
